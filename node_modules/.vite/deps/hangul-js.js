...
//#region node_modules/hangul-js/hangul.js
var require_hangul = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	/**
	* Hangul.js
	...
	*/
	(function() {
		...
		function _makeHash(array) {
			...
			for (var i = 0; i < length; i++) if (array[i]) hash[array[i].charCodeAt(0)] = i;
			return hash;
		}
		...
		function _makeComplexHash(array) {
			...
			for (var i = 0; i < length; i++) {
				...
				if (typeof hash[code1] === "undefined") hash[code1] = {};
		...
		var disassemble = function(string, grouped) {
			if (string === null) throw new Error("Arguments cannot be null");
			...
			for (var i = 0; i < length; i++) {
				...
				if (_isHangul(code)) {
					...
					temp.push(CHO[cho]);
					if (typeof JUNG[jung] === "object") temp = temp.concat(JUNG[jung]);
					else temp.push(JUNG[jung]);
					if (jong > 0) if (typeof JONG[jong] === "object") temp = temp.concat(JONG[jong]);
					else temp.push(JONG[jong]);
				} else if (_isConsonant(code)) {
					if (_isCho(code)) r = CHO[CHO_HASH[code]];
					else r = JONG[JONG_HASH[code]];
					if (typeof r === "string") temp.push(r);
					else temp = temp.concat(r);
				} else if (_isJung(code)) {
					r = JUNG[JUNG_HASH[code]];
					if (typeof r === "string") temp.push(r);
					else temp = temp.concat(r);
				} else temp.push(string.charAt(i));
		...
		var assemble = function(array) {
			if (typeof array === "string") array = disassemble(array);
			var result = [], length = array.length, code, stage = 0, complete_index = -1, previous_code, jong_joined = false;
			function _makeHangul(index) {
				var cho, jung1, jung2, jong1 = 0, jong2, hangul = "";
				jong_joined = false;
				if (complete_index + 1 > index) return;
				for (var step = 1;; step++) {
					if (step === 1) {
						cho = array[complete_index + step].charCodeAt(0);
						if (_isJung(cho)) if (complete_index + step + 1 <= index && _isJung(jung1 = array[complete_index + step + 1].charCodeAt(0))) {
						...
						if (_isCho(jung1)) {
							...
							complete_index = index;
							return;
						} else hangul = String.fromCharCode((CHO_HASH[cho] * 21 + JUNG_HASH[jung1]) * 28 + HANGUL_OFFSET);
					} else if (step === 3) {
						jung2 = array[complete_index + step].charCodeAt(0);
						if (_isJungJoinable(jung1, jung2)) jung1 = _isJungJoinable(jung1, jung2);
						else jong1 = jung2;
						...
						if (_isJongJoinable(jong1, jong2)) jong1 = _isJongJoinable(jong1, jong2);
						else jong1 = jong2;
						hangul = String.fromCharCode((CHO_HASH[cho] * 21 + JUNG_HASH[jung1]) * 28 + JONG_HASH[jong1] + HANGUL_OFFSET);
					} else if (step === 5) {
						jong2 = array[complete_index + step].charCodeAt(0);
			...
			for (var i = 0; i < length; i++) {
				...
				if (stage === 0) {
					if (_isCho(code)) stage = 1;
					else if (_isJung(code)) stage = 4;
				} else if (stage == 1) if (_isJung(code)) stage = 2;
				else if (_isJongJoinable(previous_code, code)) stage = 5;
				else _makeHangul(i - 1);
				else if (stage == 2) if (_isJong(code)) stage = 3;
				else if (_isJung(code)) if (_isJungJoinable(previous_code, code)) {} else {
					_makeHangul(i - 1);
					stage = 4;
				}
				else {
					...
					if (_isJong(code)) if (!jong_joined && _isJongJoinable(previous_code, code)) jong_joined = true;
					else {
						_makeHangul(i - 1);
						stage = 1;
					}
				...
				} else if (stage == 4) if (_isJung(code)) if (_isJungJoinable(previous_code, code)) {
					_makeHangul(i);
					stage = 0;
				} else _makeHangul(i - 1);
				else {
				...
				else if (stage == 5) if (_isJung(code)) {
					_makeHangul(i - 2);
					stage = 2;
				} else {
					_makeHangul(i - 1);
		...
		var rangeSearch = function(haystack, needle) {
			...
			while (result = re.exec(hex)) indices.push(result.index);
		...
		var endsWithConsonant = function(string) {
			...
			} else if (_isConsonant(code)) return true;
			return false;
		};
		...
		var hangul = {
			...
			isHangulAll: function(str) {
				...
				for (var i = 0; i < str.length; i++) if (!_isHangul(str.charCodeAt(i))) return false;
				return true;
			},
			...
			isConsonantAll: function(str) {
				if (typeof str !== "string") return false;
				for (var i = 0; i < str.length; i++) if (!_isConsonant(str.charCodeAt(i))) return false;
				return true;
			},
			isVowelAll: function(str) {
				if (typeof str !== "string") return false;
				for (var i = 0; i < str.length; i++) if (!_isJung(str.charCodeAt(i))) return false;
				return true;
			},
			isChoAll: function(str) {
				if (typeof str !== "string") return false;
				for (var i = 0; i < str.length; i++) if (!_isCho(str.charCodeAt(i))) return false;
				return true;
			},
			isJongAll: function(str) {
				if (typeof str !== "string") return false;
				for (var i = 0; i < str.length; i++) if (!_isJong(str.charCodeAt(i))) return false;
				return true;
			}
		};
		...
		else if (typeof module !== "undefined") module.exports = hangul;
